...
int main(int argc, char* argv[]){
  if(argc > 7){
    printf("Error! Use:\n./ppm_vbar 5 0 0 0 < input.ppm > output.ppm\n./ppm_vbar 1 255 0 0 input.ppm > output.ppm\n./ppm_vbar 15 255 255 255 input.ppm output.ppm\nWhere the first integer is the bar width, followed by its (R,G,B) color\n\n");
    exit(0);
  }
  ...
  if(argc == 7){
    ...
    image newimg = ppm_vbar(img,atoi(argv[1]),barra);
